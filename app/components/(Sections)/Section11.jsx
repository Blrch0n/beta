      ...
      <div className="w-[1140px] h-full flex flex-col gap-8">
        ...
        <div className="w-full h-fit flex justify-between items-center gap-7">
          {blogPosts.map((post, index) => (
            <div
              key={index}
              className="w-full h-[280px] flex flex-col relative overflow-hidden rounded-[15px] items-start justify-between group z-10"
            >
              <img src={post.image} className="w-full" alt="Blog Post" />
              <div className="absolute flex flex-col bottom-0 p-[25px] text-black right-0 w-full h-fit transition-all border-[#f2f2f2] border-[1px] duration-300 rounded-[15px] bg-white">
